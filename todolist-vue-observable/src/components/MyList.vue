        ...
        <ul>
            ...
            >
                <li v-for="(item) in todoLists" :key="item.id">
                    <input type="checkbox" v-model="checkedTodos" :value="item.id">
...
export default {
    ...
    computed:{
        checkedTodos:{
            ...
            set(newVal){
                todolistMutations.changeCheckTodos(newVal);
            }
        },
        todoLists:{
            get(){
    ...
    methods:{
        deleteTodo(targetTodo){
            ...
            if(flag){
                todolistMutations.deleteTodo(targetTodo)
                this.checkedTodos = this.checkedTodos.filter((item)=>{
                    return item !== targetTodo.id; 
                })
                todolistMutations.changeCheckTodos(this.checkedTodos);
            }
        },
        editTodo(item){
            item.isEdit = true;
        ...
        editDone(item){
            item.isEdit = false;
            todolistMutations.changeTodoLists(this.todoLists);
        }
    },
    mounted(){
