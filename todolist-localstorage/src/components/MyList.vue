...
export default {
    ...
    methods:{
        deleteTodo(targetTodo){
            ...
            if(flag){
                this.todoList2 = JSON.parse(window.localStorage.getItem("todoLists"));
                this.$parent.todoLists = this.todoList2 = this.todoList2.filter((item)=>{
                    return item.id !== targetTodo.id;
                })
                window.localStorage.setItem("todoLists",JSON.stringify(this.todoList2));
    ...
    watch:{
        checkedTodo:{
            handler(newVal){
                this.$parent.checkedTodos = newVal;
            }
        },
        checkedTodos:{
            handler(newVal){
